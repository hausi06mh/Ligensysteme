...
async function simulateMatch(matchId){
  ...
  try{
    ...
    // V69: Keine vorab erfundenen Chancen/Ecken mehr.
    // Nur Tore, Karten und wenige echte Standards werden geplant.
    // Chancen, Paraden, Pfosten und Ecken entstehen ausschließlich aus sichtbar abgespielten Aktionen.
    const eventControlHome=clamp(.50 + ratingGap*.035 + seasonGap*.115 + formGap*.07 + .025 - fatigueH*.025 + fatigueA*.025,.20,.80);
    // Gefährliche Freistöße bleiben geplante Unterbrechungen, zeigen aber vorher immer ein sichtbares Foul.
    const dangerousFK=clamp(poisson(1.25),0,3);
    ...
    if(!hasPenalty&&Math.random()<.10){
      ...
      if(shooter){
        if(Math.random()<.76){if(homePen)hg=Math.min(8,hg+1);else ag=Math.min(8,ag+1);addSimEvent(m,'penalty',minute(),shooter,null,0,{source:'penalty',attackingTeamId:teamId});}
...
function openLiveSimulation(matchId){
  ...
  const tempoScale=()=>1/Math.max(.85,Math.pow(speed,.68));
