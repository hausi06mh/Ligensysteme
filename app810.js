...
function simulationPowerModel(homeTeam,awayTeam,matchObj){
  ...
  let totalXg=2.82 + Math.abs(effectiveGap)*.043 + randomNormal()*.24;
  const styleRoll=Math.random();
  let style='normal';
  if(styleRoll<.028){totalXg*=1.58;style='festival'}       // sehr seltenes Torfestival
  else if(styleRoll<.155){totalXg*=1.24;style='open'}     // gelegentlich offenes Spiel
  else if(styleRoll>.855){totalXg*=.80;style='defensive'} // häufiger zähe Partien
  totalXg=clamp(totalXg,1.40,4.95);
...
async function simulateMatch(matchId){
  ...
  try{
    ...
    const addPlannedGoal=(sidePlayers,teamId)=>{
      ...
      if(r<.11)source='distance';
      else if(r<.22)source='cross';
      else if(r<.33)source='cutback';
      else if(r<.43)source='dribble';
      else if(r<.54)source='oneTwo';
      else if(r<.64)source='lowCross';
      else if(r<.74)source='rebound';
      else if(r<.84)source='secondBall';
      else if(r<.92)source='volley';
      else source='open';
...
function openLiveSimulation(matchId){
  ...
  function arrangePenalty(side,e,scored=true){
    ...
    const missMode=!scored?(Math.random()<.48?'save':Math.random()<.55?'post':'wide'):'goal';
    later(()=>{
  ...
  function boxDribbleBreak(side,carrier){
    ...
    showAction('SOLO',playerLabel(carrier),'zieht an seinem Gegenspieler vorbei',side,1700*scale);
  ...
  function oneOnOneBreak(side,carrier){
    ...
    if(runner!==carrier){
      ...
      later(()=>setBallAtNode(runner),1150*scale);
    }else setBallAtNode(runner);
    later(()=>{
      const rp=currentXY(runner),tx=side==='home'?90:10;
      showAction('ALLEIN AUFS TOR',playerLabel(runner),'nur noch der Torwart',side,1600*scale);
    ...
    later(()=>{
      ...
      realisticShot(side,runner,{kind,keeperNode:gk,stationary:true})
    },2850*scale);
    return 5400*scale;
  }
...
function ambientAttack(){
    ...
    const possessionAge=Math.max(0,simSecond-possessionStartedAt),mustProgress=possessionPassCount>=5||possessionAge>125;

    if(g==='GK'){
      // Most keeper possessions short, sometimes a sensible long distribution.
      if(Math.random()<.18){
        const target=chooseRole(side,['MID','AM','ST']);
        if(target)animatePass(carrier,target,{long:true,label:`${playerLabel(carrier)} schlägt lang`});
        else goalkeeperDistribution(side,carrier);
      }else goalkeeperDistribution(side,carrier);
      nextAmbientAction=simSecond+17+Math.random()*8;return
    }

    // Final third: many different ways to create a chance.
    if(progress>68&&['ST','AM','MID'].includes(g)){
      const r=Math.random();
      if(r<.14&&['ST','AM'].includes(g)){boxDribbleBreak(side,carrier);return}
      if(r<.25){oneOnOneBreak(side,carrier);return}
      if(r<.38){boxCombination(side,carrier);return}
      if(r<.50&&wide){crossAttack(side,carrier);return}
      if(r<.62&&['AM','MID'].includes(g)){realisticShot(side,carrier,{kind:Math.random()<.46?'save':Math.random()<.16?'post':'chance',keeperNode:keeper(side==='home'?'away':'home'),technique:'distance'});return}
      if(r<.74&&g==='ST'){dribble(carrier,side,true);nextAmbientAction=simSecond+7+Math.random()*5;return}
      if(r<.86){
        const to=choosePassTarget(carrier,side,{through:true});
        if(to){animatePass(carrier,to,{through:true});nextAmbientAction=simSecond+8+Math.random()*5;return}
      }
      realisticShot(side,carrier,{kind:Math.random()<.47?'save':Math.random()<.16?'post':'chance',keeperNode:keeper(side==='home'?'away':'home')});return
    }

    if(mustProgress){
      possessionPassCount=0;possessionStartedAt=simSecond;
      const r=Math.random();
      if(r<.19){oneOnOneBreak(side,carrier);return}
      if(r<.34&&['AM','MID','ST'].includes(g)){oneTwo(side,carrier);nextAmbientAction=simSecond+10;return}
      if(r<.48&&wide){crossAttack(side,carrier);return}
      if(r<.61&&['MID','AM'].includes(g)){boxCombination(side,carrier);return}
      if(r<.74){
        const to=choosePassTarget(carrier,side,{through:true});
        if(to){animatePass(carrier,to,{through:true});nextAmbientAction=simSecond+10+Math.random()*6;return}
      }
      if(r<.90){dribble(carrier,side,true);nextAmbientAction=simSecond+8+Math.random()*5;return}
      const opp=nearestOpp(carrier);if(opp)turnover(carrier,opp);return
    }

    // Normal build-up: passing still dominates, but not always the same lane.
    const r=Math.random();
    if(r<.08&&progress>45){counterAttack(side,carrier)}
    else if(r<.18&&['AM','ST'].includes(g)){dribble(carrier,side,false)}
    else if(r<.27&&['MID','AM','ST'].includes(g)){oneTwo(side,carrier)}
    else if(r<.34&&progress>48){oneOnOneBreak(side,carrier)}
    else if(r<.41&&progress>50&&wide){crossAttack(side,carrier)}
    else if(r<.48&&progress>55){boxCombination(side,carrier)}
    else if(r<.84){
      const long=Math.random()<.12,through=progress>48&&Math.random()<.26;
      const to=choosePassTarget(carrier,side,{through,long});
      if(to)animatePass(carrier,to,{through,long})
    }
    else if(r<.95){dribble(carrier,side,false)}
    else {const opp=nearestOpp(carrier);if(opp)turnover(carrier,opp)}

    nextAmbientAction=simSecond+16+Math.random()*11;
  }
  ...
  function guaranteedGoalSequence(side,e){
    ...
    const groups=source==='distance'?['MID','AM']:
      ...
      source==='dribble'?['ST','AM']:['ST','AM','MID'];
