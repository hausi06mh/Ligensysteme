...
function simulationPowerModel(homeTeam,awayTeam,matchObj){
  ...
  let totalXg=2.68 + Math.abs(effectiveGap)*.035 + randomNormal()*.12;
  const styleRoll=Math.random();
  let style='normal';
  if(styleRoll<.010){totalXg*=1.60;style='festival'}       // sehr seltenes Torfestival
  else if(styleRoll<.090){totalXg*=1.18;style='open'}     // gelegentlich offenes Spiel
  else if(styleRoll>.840){totalXg*=.82;style='defensive'} // häufiger zähe Partien
  totalXg=clamp(totalXg,1.40,4.60);
  const share=1/(1+Math.exp(-effectiveGap/2.75));
  let lambdaHome=clamp(totalXg*share,.18,4.1),lambdaAway=clamp(totalXg*(1-share),.16,3.9);
...
function simulateScoreFromPower(model){
  ...
  if(hg+ag>=6&&model.style!=='festival'&&Math.random()<.38){
    hg=clamp(poisson(model.lambdaHome),0,9);ag=clamp(poisson(model.lambdaAway),0,9);
  }
  if(hg+ag>=7&&model.style!=='festival'&&Math.random()<.58){
    hg=clamp(poisson(model.lambdaHome),0,9);ag=clamp(poisson(model.lambdaAway),0,9);
  }

  // 0:0 bleibt ein normales mögliches Ergebnis, aber nicht übermäßig häufig.
  if(hg===0&&ag===0&&model.style!=='defensive'&&Math.random()<.45){
...
async function simulateMatch(matchId){
  ...
  try{
    ...
    const addPlannedGoal=(sidePlayers,teamId)=>{
      ...
      if(r<.16)source='distance';
      else if(r<.38)source='cross';
      else if(r<.56)source='cutback';
      else if(r<.70)source='dribble';
      else source='open';
    ...
    const foulCount=clamp(poisson(11.5),7,17),booked=new Set();
    for(let i=0;i<foulCount;i++){
      ...
      const fm=minute(),attackingTeamId=foulingHome?m.awayId:m.homeId,inBox=Math.random()<.055,dangerous=!inBox&&Math.random()<.24,roll=Math.random();let cardType=null;
...
function openLiveSimulation(matchId){
  ...
  el('#overlay').innerHTML=`<div class="modal live2d-modal"><div class="live2d-shell live2d-v50 live2d-v52 live2d-v54 live2d-v55 live2d-v56 live2d-v57 live2d-v58 live2d-v59 live2d-v60 live2d-v65 live2d-v68 live2d-v76 live2d-v77 live2d-v78" style="--liveTempo:1.05s;--home:${hColor};--away:${aColor};${h.stadium?.image?`--stadium-photo:url(\'${h.stadium.image}\');`:``}">
    ...
    <section class="v50-stadium-wrap v53-stadium-wrap v54-stadium-wrap">
      ...
      <section class="live2d-pitch v50-pitch v52-pitch" id="live2dPitch" style="--home:${hColor};--away:${aColor}">
        ...
        <div class="live2d-shot-result" id="live2dShotResult" aria-live="polite"><b></b><span></span></div>
  ...
  const shell=document.querySelector('.live2d-shell'),pitch=el('#live2dPitch'),clock=el('#live2dClock'),score=el('#live2dScore'),period=el('#live2dPeriod'),progress=el('#live2dProgress'),ticker=el('#live2dTicker'),scene=el('#live2dScene'),ball=el('#live2dBall'),goalFlash=el('#live2dGoalFlash'),goalName=el('#live2dGoalName'),shotResult=el('#live2dShotResult'),setpiece=el('#live2dSetpiece'),tickerPanel=el('#live2dTickerPanel'),statsPanel=el('#live2dStatsPanel'),traceMain=el('#live2dTraceMain'),attackBar=el('#live2dAttackBar'),attackTeam=el('#live2dAttackTeam'),possFlash=el('#live2dPossFlash'),actionCard=el('#live2dActionCard'),actionType=el('#live2dActionType'),actionTitle=el('#live2dActionTitle'),actionDetail=el('#live2dActionDetail'),statusRail=el('#live2dStatusRail'),statusType=el('#live2dStatusType'),statusTitle=el('#live2dStatusTitle'),statusDetail=el('#live2dStatusDetail'),halftimePanel=el('#live2dHalftimePanel');
  
  // V77: Stadion-/Broadcast-Layout. Bestehende echte Bedienelemente werden
  // an die seitlichen Leisten verschoben, damit nichts nur Dekoration ist.
  const v77Stadium=shell?.querySelector('.v50-stadium-wrap');
  const v77Board=shell?.querySelector('.v50-scoreboard');
  const v77Controlbar=shell?.querySelector('.v50-controlbar');
  if(v77Stadium){
    const v78InfoToggle=document.createElement('button');
    v78InfoToggle.type='button';
    v78InfoToggle.id='v78InfoToggle';
    v78InfoToggle.className='v78-info-toggle';
    v78InfoToggle.textContent='☷';
    v78InfoToggle.setAttribute('aria-label','Statistiken und Ticker ein-/ausblenden');
    v77Stadium.appendChild(v78InfoToggle);

    if(v77Board)v77Stadium.prepend(v77Board);
    if(v77Controlbar)v77Stadium.appendChild(v77Controlbar);
    const leftRail=document.createElement('div');leftRail.className='v77-side-rail v77-left-rail';
    const brand=document.createElement('div');brand.className='v77-rail-brand';brand.textContent='⚽';leftRail.appendChild(brand);
    const pauseBtn=el('#live2dPause'),speedBtn=el('#live2dSpeed'),cameraBtn=el('#v50Camera');
    if(pauseBtn)leftRail.appendChild(pauseBtn);if(speedBtn)leftRail.appendChild(speedBtn);if(cameraBtn)leftRail.appendChild(cameraBtn);
    const rightRail=document.createElement('div');rightRail.className='v77-side-rail v77-right-rail';
    const closeBtn=el('#live2dClose'),gearBtn=el('#v50Gear'),tacticsBtn=el('#v50Tactics');
    if(closeBtn)rightRail.appendChild(closeBtn);if(gearBtn)rightRail.appendChild(gearBtn);if(tacticsBtn)rightRail.appendChild(tacticsBtn);
    v77Stadium.append(leftRail,rightRail);
  }
const v78InfoToggle=el('#v78InfoToggle');
  if(v78InfoToggle){
    v78InfoToggle.addEventListener('click',()=>{
      shell.classList.toggle('v78-info-open');
      v78InfoToggle.textContent=shell.classList.contains('v78-info-open')?'×':'☷';
    });
  }
  ...
  function setBallXY(x,y,air=false){if(ball){ball.classList.remove('behind-goal');ball.style.opacity='1'}ballCarrierNode=null;if(ballFlightRAF){cancelAnimationFrame(ballFlightRAF);ballFlightRAF=0}rawBallXY(x,y,air)}
  function actualXY(node){return currentXY(node)}
  function setBallAtNode(node){if(!node)return;if(ball){ball.classList.remove('behind-goal');ball.style.opacity='1'}const newSide=sideOf(node);if(newSide&&newSide!==lastPossessionSide){possessionPassCount=0;possessionStartedAt=simSecond;lastPossessionSide=newSide;}ballCarrierNode=node;currentBallPlayerId=playerNodeId(node);ballInFlight=false;shell?.querySelectorAll('.live2d-player.ball-owner').forEach(n=>n.classList.remove('ball-owner'));node.classList.add('ball-owner');const p=actualXY(node);rawBallXY(p.x,p.y,false)}
  ...
  function flyBall(start,getEnd,duration=900,{air=false,kind='pass',onDone=null,allowOutside=false}={}){
    if(ballFlightRAF)cancelAnimationFrame(ballFlightRAF);clearBallOwner();ballInFlight=true;
    const paint=(x,y,isAir=false)=>{
      if(!ball)return;
      ball.classList.toggle('air',isAir);
      ball.style.left=`${allowOutside?x:clamp(x,1.3,98.7)}%`;
      ball.style.top=`${allowOutside?y:clamp(y,2,98)}%`;
    };
    const t0=performance.now(),end0=typeof getEnd==='function'?getEnd():getEnd;
    drawTrace(start,end0,kind,Math.max(450,duration*.9));
    const step=now=>{
      const t=clamp((now-t0)/Math.max(120,duration),0,1),e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2,end=typeof getEnd==='function'?getEnd():getEnd;
      let x=start.x+(end.x-start.x)*e,y=start.y+(end.y-start.y)*e;
      if(air)y-=Math.sin(Math.PI*t)*5.5;
      paint(x,y,air);
      if(t<1)ballFlightRAF=requestAnimationFrame(step);
      else{ballFlightRAF=0;ballInFlight=false;paint(end.x,end.y,false);onDone?.()}
    };
  ...
  function updateDynamicShape(force=false){
    ...
    const now=performance.now();if(!force&&now<nextShapeReal)return;nextShapeReal=now+1750/Math.max(.96,Math.sqrt(speed));shapeNonce++;
    ...
    ['home','away'].forEach(side=>{
      ...
      sideNodes(side).forEach(node=>{
        ...
        const block=clamp((ballProg-50)*(own?.052:.036),-2.3,3.0);if(g!=='GK')x+=dir*block;
        if(g!=='GK')y+=clamp((cp.y-b.y)*(own?.025:.017),-1.35,1.35);
        if(own){
  ...
  function runTo(node,x,y,ms=1200,cls='runner'){
    if(!node)return;const safeMs=Math.max(1050,Number(ms)||1450);node.classList.add(cls);setLocked(node,safeMs+180);setNodeXY(node,x,y);later(()=>node.classList.remove(cls),safeMs+120);
  }
  ...
  function arrangeCorner(side,e,{causeAlreadyShown=false}={}){
    ...
    const takeCorner=()=>{
      ...
      later(()=>{showAction('KOPFBALL',playerLabel(aerial),'kommt zum Abschluss',side,1150*scale);const r=Math.random();const kind=r<.09?'goal':r<.44?'save':r<.55?'post':r<.69?'block':'chance';if(kind==='goal'){
        ...
        realisticShot(side,aerial,{kind:'goal',keeperNode:gk,event:goalEvent,technique:'header'});later(()=>playGoalAnimation(goalEvent,side,aerial),1650*scale);
  ...
  function arrangeFreeKick(side,e={}){
    const scale=tempoScale(),other=side==='home'?'away':'home',dir=attackingDir(side),taker=nodeFor(e?.playerId)||chooseRole(side,['AM','MID','ST']),gk=keeper(other),dangerous=Boolean(e?.dangerous),ballX=side==='home'?(dangerous?70+Math.random()*8:55+Math.random()*12):(dangerous?30-Math.random()*8:45-Math.random()*12),ballY=28+Math.random()*44,goalX=side==='home'?98:2;
    ...
    later(()=>{const outcome=Math.random()<.45?'save':Math.random()<.20?'post':'chance';realisticShot(side,taker,{kind:outcome,keeperNode:gk,event:e,stationary:true});},2200*scale);
  ...
  function v76ShowShotResult(kind,side,title,detail=''){
    if(!shotResult)return;
    shotResult.className=`live2d-shot-result show ${kind} ${side}`;
    const b=shotResult.querySelector('b'),sp=shotResult.querySelector('span');
    if(b)b.textContent=title;if(sp)sp.textContent=detail;
    clearTimeout(v76ShowShotResult.timer);
    v76ShowShotResult.timer=setTimeout(()=>shotResult.classList.remove('show'),1500*tempoScale());
  }
  function v76GoalFrame(side,kind='shot'){
    const goalSide=side==='home'?'right':'left';
    const node=shell?.querySelector(`.live2d-field-lines .goal.${goalSide}`);
    if(!node)return;
    node.classList.remove('v76-goal','v76-save','v76-post','v76-miss');
    void node.offsetWidth;
    node.classList.add(`v76-${kind}`);
    later(()=>node.classList.remove(`v76-${kind}`),1200*tempoScale());
  }
  function v76BallBehindGoal(side,y,after=null){
    if(!ball){after?.();return}
    ball.classList.add('v76-terminal-flight');
    ball.style.opacity='1';
    ball.style.left=side==='home'?'103%':'-3%';
    ball.style.top=`${clamp(y,4,96)}%`;
    later(()=>{if(ball){ball.style.opacity='0';ball.classList.remove('v76-terminal-flight')}after?.()},330*tempoScale());
  }
  function v76ShotOutcome(){
    const r=Math.random();
    if(r<.14)return 'block';
    if(r<.54)return 'save';
    if(r<.63)return 'post';
    return 'chance';
  }

function realisticShot(side,shooter,{kind='chance',keeperNode=null,event=null,technique='auto',stationary=false}={}){
    const scale=tempoScale(),other=side==='home'?'away':'home',goalX=side==='home'?98.45:1.55,gk=keeperNode||keeper(other);
    actionBusyUntil=performance.now()+4600*scale;
    if(!shooter)return;

    const sp=actualXY(shooter),shootX=side==='home'?clamp(Math.max(sp.x,69),69,88):clamp(Math.min(sp.x,31),12,31),shootY=clamp(sp.y,16,84);
    const distance=Math.round(Math.abs((side==='home'?100-shootX:shootX))*1.05);
    if(!stationary)runTo(shooter,shootX,shootY,760*scale,'shooter');
    setLocked(shooter,4700*scale);setBallAtNode(shooter);setAttackLabel(side,'ABSCHLUSS');
    ...
    showAction(shotType,playerLabel(shooter),`${distance} m`,side,1450*scale);
    showScene(`${playerLabel(shooter)} zieht ab`,true);

    later(()=>{
      const start=actualXY(shooter);
      const onTargetY=42+Math.random()*16;
      const missMode=Math.random()<.62?'wide':'over';
      const missY=missMode==='wide'?(Math.random()<.5?28+Math.random()*6:66+Math.random()*6):(44+Math.random()*12);
      const postY=Math.random()<.5?39.5:60.5;
      const defender=kind==='block'?nearestOpp(shooter):null;

      // Block: Gegner bewegt sich wirklich in die Schussbahn, Ball prallt sichtbar ab.
      if(kind==='block'&&defender){
        const dp=actualXY(defender);
        const bx=clamp(start.x+(goalX-start.x)*.42,side==='home'?73:12,side==='home'?91:27);
        const by=clamp(start.y+(onTargetY-start.y)*.42,16,84);
        runTo(defender,bx,by,620*scale,'duel');setLocked(defender,2600*scale);
        later(()=>{
          flyBall(actualXY(shooter),()=>actualXY(defender),610*scale,{kind:'shot',onDone:()=>{
            registerVisibleEvent('chance',side,event);
            const faux=event||persistLiveEvent('chance',side,{playerId:playerNodeId(shooter),source:'blocked-shot',extra:{outcome:'block'}});
            ...
            v76ShowShotResult('block',side,'GEBLOCKT',`${playerLabel(defender)} wirft sich dazwischen`);
            showAction('GEBLOCKT',playerLabel(defender),`blockt den Schuss von ${playerLabel(shooter)}`,other,1800*scale);
            showScene(`🛡 ${playerLabel(defender)} blockt den Schuss`,true);
            const reboundSide=Math.random()<.44?side:other;
            const rebound=chooseRole(reboundSide,['MID','AM','DEF','ST'])||chooseCarrier(reboundSide);
            const rp=rebound?actualXY(rebound):{x:50,y:50};
            flyBall(actualXY(defender),rp,460*scale,{kind:'miss',onDone:()=>{
              possessionSide=reboundSide;possessionPassCount=0;possessionStartedAt=simSecond;
              if(rebound)setBallAtNode(rebound);
              nextAmbientAction=simSecond+5+Math.random()*5;
            }});
          }});
        },540*scale);
        return;
      ...
      const end=kind==='chance'?{x:goalX,y:missY}:kind==='post'?{x:goalX,y:postY}:{x:goalX,y:onTargetY};

      // Keeper bewegt sich nur bei einem tatsächlich aufs Tor kommenden Ball.
      if(gk&&['save','goal'].includes(kind)){
        runTo(gk,side==='home'?96.2:3.8,onTargetY,700*scale,'runner');
        setLocked(gk,3400*scale);
      }

      flyBall(start,end,850*scale,{
        air:technique==='header'||distance>18,
        kind:kind==='chance'?'miss':kind==='post'?'post':'shot',
        onDone:()=>{
          // TOR: Ball überquert sichtbar die Linie und verschwindet im/ hinter dem Netz.
          if(kind==='goal'){
            v76GoalFrame(side,'goal');
            v76ShowShotResult('goal',side,'TOR!','Ball im Netz');
            showScene('⚽ Der Ball ist im Netz!',true);
            v76BallBehindGoal(side,onTargetY);
            return;
          }

          // PARADE: Ball endet sichtbar beim Keeper oder wird zur Ecke abgewehrt.
          if(kind==='save'&&gk){
            registerVisibleEvent('save',side,event);
            const faux=event||persistLiveEvent('save',side,{playerId:playerNodeId(gk),shotById:playerNodeId(shooter),source:'open-play'});
            if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
            v76GoalFrame(side,'save');
            const parry=Math.random()<.27;
            if(parry){
              v76ShowShotResult('save',side,'PARADE!',`${playerLabel(gk)} lenkt zur Ecke`);
              showAction('PARADE',playerLabel(gk),'lenkt den Ball zur Ecke',other,1850*scale);
              showScene(`🧤 ${playerLabel(gk)} pariert zur Ecke`,true);
              const deflectY=Math.random()<.5?8:92;
              flyBall({x:goalX,y:onTargetY},{x:side==='home'?100.8:-.8,y:deflectY},460*scale,{kind:'miss',allowOutside:true,onDone:()=>{
                if(ball)ball.style.opacity='0';
                later(()=>arrangeCorner(side,{playerId:playerNodeId(chooseRole(side,['AM','MID'])),attackingTeamId:side==='home'?m.homeId:m.awayId,source:'save'},{causeAlreadyShown:true}),350*scale);
              }});
            }else{
              setBallAtNode(gk);
              v76ShowShotResult('save',side,'GEHALTEN',`${playerLabel(gk)} hält sicher`);
              possessionSide=other;possessionPassCount=0;possessionStartedAt=simSecond;
              showAction('PARADE',playerLabel(gk),`hält den Schuss von ${playerLabel(shooter)}`,other,1850*scale);
              showScene(`🧤 ${playerLabel(gk)} hält sicher`,true);
              nextAmbientAction=simSecond+12+Math.random()*6;
            }
            return;
          }

          // PFOSTEN/LATTE: erst Kontakt mit dem Rahmen, dann sichtbarer Abpraller.
          if(kind==='post'){
            registerVisibleEvent('post',side,event);
            const faux=event||persistLiveEvent('post',side,{playerId:playerNodeId(shooter),source:'open-play'});
            if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
            v76GoalFrame(side,'post');
            v76ShowShotResult('post',side,'PFOSTEN!',`${playerLabel(shooter)} trifft Aluminium`);
            showAction('ALUMINIUM',playerLabel(shooter),'Pfosten oder Latte',side,1900*scale);
            showScene('🥅 Aluminium!',true);
            const keepAttack=Math.random()<.40;
            const reboundSide=keepAttack?side:other;
            const rebound=chooseRole(reboundSide,keepAttack?['ST','AM','MID']:['DEF','MID'])||gk||chooseCarrier(reboundSide);
            const target=rebound?actualXY(rebound):{x:50,y:50};
            flyBall({x:goalX,y:postY},target,560*scale,{kind:'post',onDone:()=>{
              possessionSide=reboundSide;possessionPassCount=0;possessionStartedAt=simSecond;
              if(rebound)setBallAtNode(rebound);
              nextAmbientAction=simSecond+(keepAttack?4:9)+Math.random()*4;
            }});
            return;
          }

          // DANEBEN / DRÜBER: Ball fliegt sichtbar über die Grundlinie und verschwindet.
          registerVisibleEvent('chance',side,event);
          const faux=event||persistLiveEvent('chance',side,{playerId:playerNodeId(shooter),source:'open-play',extra:{outcome:missMode}});
          if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
          v76GoalFrame(side,'miss');
          const label=missMode==='wide'?'DANEBEN':'DRÜBER';
          v76ShowShotResult('miss',side,label,'Abstoß');
          showAction(label,playerLabel(shooter),missMode==='wide'?'am Tor vorbei':'über das Tor',side,1750*scale);
          showScene(`↗ ${playerLabel(shooter)} schießt ${missMode==='wide'?'vorbei':'drüber'} – Abstoß`,true);
          v76BallBehindGoal(side,missY,()=>{
            possessionSide=other;possessionPassCount=0;possessionStartedAt=simSecond;
            const outlet=gk||chooseRole(other,['DEF','MID']);
            if(outlet)setBallAtNode(outlet);
            nextAmbientAction=simSecond+10+Math.random()*5;
          });
        }
      });
    },stationary?260*scale:820*scale);
  }
  ...
  function crossAttack(side,carrier){
    const scale=tempoScale(),p=currentXY(carrier),target=chooseRole(side,['ST','AM'],[carrier]),other=side==='home'?'away':'home',gk=keeper(other);if(!target)return 0;
  ...
  function ambientAttack(){
    ...
    if(!carrier)return;
    const side=possessionSide,g=groupOf(carrier),progress=progressFor(carrier,side),wide=Math.abs(currentXY(carrier).y-50)>24;
    updateDynamicShape(true);
    const possessionAge=Math.max(0,simSecond-possessionStartedAt),mustProgress=possessionPassCount>=4||possessionAge>115;

    // Torwart spielt kontrolliert zum eigenen Verteidiger/Mittelfeldspieler – niemals zum Gegner.
    if(g==='GK'){goalkeeperDistribution(side,carrier);nextAmbientAction=simSecond+18+Math.random()*8;return}

    // Im letzten Drittel MUSS der Angriff zu einer klaren Aktion führen.
    if(progress>70&&['ST','AM','MID'].includes(g)){
      if(wide&&Math.random()<.48){crossAttack(side,carrier);return}
      if(['AM','MID'].includes(g)&&progress<82&&Math.random()<.32){realisticShot(side,carrier,{kind:v76ShotOutcome(),keeperNode:keeper(side==='home'?'away':'home'),technique:'distance'});return}
      if(g==='ST'&&Math.random()<.24){dribble(carrier,side,true);nextAmbientAction=simSecond+8+Math.random()*5;return}
      realisticShot(side,carrier,{kind:v76ShotOutcome(),keeperNode:keeper(side==='home'?'away':'home')});return
    }

    // Nach mehreren Pässen: vertikaler Vorstoß statt weiterer Querpass-Schleife.
    if(mustProgress){
      possessionPassCount=0;possessionStartedAt=simSecond;
      if(wide&&progress>55&&Math.random()<.50){crossAttack(side,carrier);return}
      if(['MID','AM'].includes(g)&&progress>52&&Math.random()<.35){realisticShot(side,carrier,{kind:v76ShotOutcome(),keeperNode:keeper(side==='home'?'away':'home'),technique:'distance'});return}
      const to=choosePassTarget(carrier,side,{through:true});if(to){animatePass(carrier,to,{through:true});nextAmbientAction=simSecond+11+Math.random()*7;return}
      dribble(carrier,side,true);nextAmbientAction=simSecond+9+Math.random()*6;return
    }

    const r=Math.random();
    if(progress>58&&wide&&r<.18){crossAttack(side,carrier)}
    else if(['AM','ST'].includes(g)&&r<.30){dribble(carrier,side,progress>55)}
    else if(['MID','AM','ST'].includes(g)&&r<.40){oneTwo(side,carrier)}
    else if(r<.80){const through=progress>52&&Math.random()<.22;const to=choosePassTarget(carrier,side,{through});if(to)animatePass(carrier,to,{through})}
    else if(r<.94){dribble(carrier,side,false)}
    else {const opp=nearestOpp(carrier);if(opp)turnover(carrier,opp)}
    nextAmbientAction=simSecond+18+Math.random()*12;
  }
  ...
  function guaranteedGoalSequence(side,e){
    ...
    const groups=source==='distance'?['MID','AM']:source==='cross'?['ST','AM','MID']:source==='cutback'?['MID','AM','ST']:source==='dribble'?['ST','AM']:['ST','AM','MID'];
  ...
  function focusEvent(e){
    ...
    if(e.type==='foul'){
      ...
      later(()=>{
        ...
        showAction('FOUL',playerLabel(fouler),`stoppt ${playerLabel(victim)}`,foulingSide,1350*scale);showScene(`Pfiff – ${playerLabel(victim)} wird gefoult`,true);
      },850*scale);
    ...
    try{
      if(!paused&&!ended){
        ...
        if(eventSceneActive&&eventSceneUntil&&perf>eventSceneUntil){
          ...
          eventSceneActive=false;restartLock=false;actionBusyUntil=0;cinematicUntil=0;ballInFlight=false;
        }
      ...
      eventSceneActive=false;eventSceneUntil=0;restartLock=false;cinematicUntil=0;actionBusyUntil=0;ballInFlight=false;
    }
