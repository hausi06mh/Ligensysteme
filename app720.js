...
function openLiveSimulation(matchId){
  ...
  function persistLiveEvent(type,side,{playerId=null,shotById=null,assistId=null,source='live',extra={}}={}){
    const minute=Math.max(1,Math.min(90,Math.floor(simSecond/60)));
    // Planned event already exists: don't duplicate it.
    const duplicate=(m.events||[]).find(ev=>ev.type===type&&Number(ev.minute||0)===minute&&Number(ev.playerId||0)===Number(playerId||0)&&String(ev.source||'')===String(source||''));
    if(duplicate)return duplicate;
    const ev={id:nextId(m.events),type,minute,addedTime:0,playerId:Number(playerId)||null,assistId:Number(assistId)||null,playerOutId:null,shotById:Number(shotById)||null,attackingTeamId:side==='home'?m.homeId:m.awayId,source,...extra};
    ...
    later(()=>{
      ...
      flyBall(start,end,820*scale,{air:technique==='header'||distance>18,kind:kind==='chance'?'miss':kind==='post'?'post':'shot',onDone:()=>{
        ...
        if(kind==='save'&&gk){
          ...
          registerVisibleEvent('post',side,event);const faux=event||persistLiveEvent('post',side,{playerId:playerNodeId(shooter),source:'open-play'});if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
          ...
          registerVisibleEvent('chance',side,event);const faux=event||persistLiveEvent('chance',side,{playerId:playerNodeId(shooter),source:'open-play'});if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
          if(ball){ball.classList.add('behind-goal');ball.style.opacity='0'}showAction('VORBEI',playerLabel(shooter),'Ball geht hinter das Tor',side,1750*scale);showScene(`↗ ${playerLabel(shooter)} schießt vorbei – Abstoß`,true);possessionSide=other;possessionPassCount=0;possessionStartedAt=simSecond;const outlet=gk||chooseRole(other,['DEF','MID']);if(outlet)setBallAtNode(outlet);nextAmbientAction=simSecond+10+Math.random()*5;
        }
      }})
