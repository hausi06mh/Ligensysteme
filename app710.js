...
function openLiveSimulation(matchId){
  ...
  function updateDynamicShape(force=false){
    ...
    ['home','away'].forEach(side=>{
      ...
      sideNodes(side).forEach(node=>{
        ...
        const block=clamp((ballProg-50)*(own?.065:.045),-2.8,3.8);if(g!=='GK')x+=dir*block;
  ...
  function focusEvent(e){
    ...
    if(e.type==='foul'){
      const fouler=nodeFor(e.playerId),victim=nodeFor(e.victimId),foulingSide=Number(e.foulingTeamId||0)===Number(m.homeId)?'home':'away',attackSide=foulingSide==='home'?'away':'home';restartLock=true;eventSceneActive=true;
      if(fouler&&victim){const v=currentXY(victim);runTo(fouler,v.x+(Math.random()-.5)*1.2,v.y+(Math.random()-.5)*1.2,1100*scale,'duel');victim.classList.add('duel');setLocked(victim,6500*scale)}
      registerVisibleEvent('foul',foulingSide,e);showAction('FOUL',fouler?playerLabel(fouler):'Foul',victim?`an ${playerLabel(victim)}`:'Pfiff',foulingSide,1500*scale);showScene(`Pfiff – Foul${victim?` an ${playerLabel(victim)}`:''}`,true);
      const continueAfterCard=()=>{victim?.classList.remove('duel');if(e.inBox){showAction('ELFMETER!',teamLabel(attackSide),'Foul im Strafraum',attackSide,1500*scale);showScene('Der Schiedsrichter zeigt auf den Punkt!',true);const sh=chooseRole(attackSide,['ST','AM','MID'])||victim||chooseCarrier(attackSide),pe={...e,type:e.penaltyScored?'penalty':'missedPenalty',playerId:playerNodeId(sh),attackingTeamId:attackSide==='home'?m.homeId:m.awayId,source:'foul'};later(()=>arrangePenalty(attackSide,pe,Boolean(e.penaltyScored)),950*scale);later(()=>{restartLock=false;eventSceneActive=false},6500*scale)}else{showAction('FREISTOSS',teamLabel(attackSide),e.dangerous?'gute Position':'nach dem Foul',attackSide,1350*scale);showScene('Freistoß nach dem Foul',true);const t=chooseRole(attackSide,['AM','MID','ST'])||victim||chooseCarrier(attackSide),x={...e,type:'freeKick',playerId:playerNodeId(t),attackingTeamId:attackSide==='home'?m.homeId:m.awayId,source:'foul',dangerous:Boolean(e.dangerous)};later(()=>arrangeFreeKick(attackSide,x),700*scale);later(()=>{restartLock=false;eventSceneActive=false},6200*scale)}};
      later(()=>{if(e.cardType){const label=e.cardType==='yellow'?'GELBE KARTE':e.cardType==='secondYellow'?'GELB-ROT':'ROTE KARTE';showAction(label,fouler?playerLabel(fouler):'Spieler','nach dem Foul',foulingSide,1800*scale);showScene(`${label} nach dem Foul`,true);const ce={...e,type:e.cardType,source:'foul'},tt=liveTickerDisplayTitle(ce,m);tickerRows.unshift(liveSimTickerRow(ce,tt,foulingSide));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('');if(fouler&&['red','secondYellow'].includes(e.cardType))later(()=>fouler.classList.add('sent-off'),900*scale);later(continueAfterCard,1500*scale)}else continueAfterCard()},1450*scale);return
    }
