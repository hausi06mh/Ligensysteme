...
function openLiveSimulation(matchId){
  ...
  function flyBall(start,getEnd,duration=900,{air=false,kind='pass',onDone=null}={}){
    if(ballFlightRAF)cancelAnimationFrame(ballFlightRAF);clearBallOwner();ballInFlight=true;
    const t0=performance.now(),target0=typeof getEnd==='function'?getEnd():getEnd;
    ...
    const step=now=>{
      ...
      else{ballFlightRAF=0;ballInFlight=false;rawBallXY(end.x,end.y,false);onDone?.()}
    };
  ...
  function boxDribbleBreak(side,carrier){
    ...
    showAction('SOLO',playerLabel(carrier),soloText,side,1700*scale);
...
function ambientAttack(){
    ...
    clearSequenceTimers();
    if(ballFlightRAF){cancelAnimationFrame(ballFlightRAF);ballFlightRAF=0;ballInFlight=false;}
    ...
    if(g==='GK'){
      ...
      nextAmbientAction=simSecond+12+Math.random()*7;return
    }

    // Final third: many different ways to create a chance.
    if(progress>68&&['ST','AM','MID'].includes(g)){
      const r=Math.random();
      if(r<.18&&['ST','AM'].includes(g)){boxDribbleBreak(side,carrier);return}
      if(r<.34){oneOnOneBreak(side,carrier);return}
      if(r<.49){boxCombination(side,carrier);return}
      if(r<.61&&wide){crossAttack(side,carrier);return}
      if(r<.74&&['AM','MID','ST'].includes(g)){realisticShot(side,carrier,{kind:Math.random()<.48?'save':Math.random()<.14?'post':'chance',keeperNode:keeper(side==='home'?'away':'home'),technique:progress>76?'auto':'distance'});return}
      if(r<.84&&['ST','AM'].includes(g)){dribble(carrier,side,true);nextAmbientAction=simSecond+5+Math.random()*4;return}
      if(r<.93){
    ...
    nextAmbientAction=simSecond+10+Math.random()*8;
  }
