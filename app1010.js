...
function v101Club(t){const q=v101Load().clubs[String(t.id)]||{};return{name:q.name||t.name,stadium:q.stadium||t.stadium||'Vereinsstadion',capacity:Number(q.capacity||t.capacity||t.stadiumCapacity||12000),strength:Number(q.strength||Math.round(v100TeamAverage(t)||teamAverage(t)||66))}}
...
function v101EditWorld(l){const x=v101Load(),c=v101WorldCfg(l),name=prompt('LigName:',c.name);if(name===null)return;let size=Math.max(4,Math.min(24,Number(prompt('Anzahl Teams:',c.size))||c.size)),teams=c.teams.slice(0,size);while(teams.length<size)teams.push({id:`${l.id}-c${teams.length}`,name:`Team ${teams.length+1}`,strength:65});x.world[l.id]={name:name.trim()||c.name,size,teams};v101Save(x);render()}
function v101EditWorldTeam(l,i){const x=v101Load(),c=v101WorldCfg(l),t=c.teams[i];if(!t)return;const name=prompt('Mannschaftsname:',t.name);if(name===null)return;const st=prompt('Teamstärke 40–99:',t.strength);if(st===null)return;t.name=name.trim()||t.name;t.strength=Math.max(40,Math.min(99,Number(st)||t.strength));x.world[l.id]=c;v101Save(x);render()}
  ...
  el("#app").innerHTML=`
    <div class="shell v96-shell">
      <header class="topbar"><div class="topbar-inner">
        <div class="brand"><div class="brandmark">🏆</div><div>Fantasy Liga Elite <span class="version-pill">V101 · Custom</span></div></div>
...
function renderFootballWorld(){
 ...
 <header class="v91-hero"><div><div class="v91-kicker">FUSSBALLWELT</div><h1>Die Welt läuft weiter.</h1><p>Mehrere Ligen werden parallel zu deiner Saison simuliert.</p></div><div class="v91-day"><small>DEIN SPIELTAG</small><b>${Number(getSeason()?.currentMatchday||getSeason()?.matchday||w.matchday||0)}</b></div></header>
 <nav class="v91-league-tabs">${Object.values(w.leagues).map(x=>`<button data-world-league="${x.id}" class="${x.id===l.id?'active':''}"><small>${x.country}</small><b>${v101WorldCfg(x).name}</b></button>`).join('')}</nav>
 <div class="v91-head"><div><span>${l.country}</span><h2>${v101WorldCfg(l).name}</h2></div><strong>Saison ${l.season}</strong></div>
 ...
 <section class="v101-world-editor"><h3>4 zusätzliche Ligen bearbeiten</h3>${Object.values(w.leagues).slice(0,4).map(q=>{const c=v101WorldCfg(q);return `<article><div><small>${c.size} Teams</small><b>${c.name}</b></div><button data-v101-world="${q.id}">Bearbeiten</button></article>`}).join('')}</section><section class="v101-world-teams">${v101WorldCfg(l).teams.map((t,i)=>`<button data-v101-world-team="${i}"><span>${t.name}</span><b>${t.strength}</b></button>`).join('')}</section><div class="v91-grid">
 ...
 content.querySelectorAll('[data-v101-world]').forEach(b=>b.onclick=()=>{const q=w.leagues[b.dataset.v101World];if(q)v101EditWorld(q)});
...
function renderClubProfile(){
 ...
 const fixtures=getSeason()?.fixtures||[],next=fixtures.find(f=>!(f.played||f.done||f.status==='played')&&(String(f.homeId)===String(team.id)||String(f.awayId)===String(team.id))),opp=next?v90Team(String(next.homeId)===String(team.id)?next.awayId:next.homeId):null;
 ...
 <section class="v101-club-tools"><div><small>STADION</small><b>${v101Club(team).stadium}</b><span>${v101Club(team).capacity.toLocaleString('de-DE')} Plätze · Stärke ${v101Club(team).strength}</span></div><button data-v101-edit-club>Bearbeiten</button></section><button class="v98-open" data-v98-open>⇄ Transfers & Entwicklung</button><div class="v90-profile-grid"><article class="v90-card"><h3>Saisonbilanz</h3><div class="v90-numbers"><div><b>${rec.p}</b><span>Spiele</span></div><div><b>${rec.w}</b><span>Siege</span></div><div><b>${rec.d}</b><span>Remis</span></div><div><b>${rec.l}</b><span>Niederlagen</span></div><div><b>${rec.gf}:${rec.ga}</b><span>Tore</span></div></div></article>
 ...
 <article class="v90-card v101-player-editor"><h3>Spieler bearbeiten</h3><div>${v100VisibleSquad(team).map((pl,i)=>{const q=v101Player(team,pl,i);return `<button data-v101-edit-player="${i}"><span><b>${q.name}</b><small>${q.position}</small></span><strong>${q.rating}</strong></button>`}).join('')}</div></article><article class="v90-card"><h3>Kader</h3><div class="v90-squad-list">${squad.length?squad.slice().sort((a,b)=>Number(b.rating??b.overall??0)-Number(a.rating??a.overall??0)).map(p=>`<button data-player="${String(p.id??p.playerId??p.name).replace(/"/g,'&quot;')}"><span><b>${p.name}</b><small>${p.position||p.pos||'Spieler'}</small></span><strong>${p.rating??p.overall??'–'}</strong></button>`).join(''):'<p class="muted">Keine Kaderspieler hinterlegt.</p>'}</div></article>
...
function renderHome(){
  ...
  const fixtures=season.fixtures||[],played=fixtures.filter(f=>f.played||f.done||f.status==='played');
  const clubPlayed=played.filter(f=>f.homeId===club.id||f.awayId===club.id);
  const last=clubPlayed.slice(-1)[0];
  const next=fixtures.find(f=>!(f.played||f.done||f.status==='played')&&(f.homeId===club.id||f.awayId===club.id));
  ...
  content.innerHTML=`
    <section class="manager-hub">${v101Picker()}<div class="mgr-hero">
      ...
      <section class="v101-stadium"><div><small>STADION</small><b>${v101Club(club).stadium}</b></div><strong>${v101Club(club).capacity.toLocaleString('de-DE')} <span>Plätze</span></strong></section><div class="mgr-grid">
