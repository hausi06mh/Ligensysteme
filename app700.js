...
async function simulateMatch(matchId){
  ...
  try{
    ...
    // V70: Fouls sind jetzt die Quelle für Karten UND Freistöße. Keine zufälligen Karten/Standards mehr.
    const foulCount=clamp(poisson(12.5),7,18);
    const booked=new Set();
    for(let i=0;i<foulCount;i++){
      const foulingHome=Math.random()<.50;
      const foulingPlayers=foulingHome?hXI:aXI, victimPlayers=foulingHome?aXI:hXI;
      const fouler=weightedPick(foulingPlayers,p=>{const g=playerPositionGroup(p);return g==='DEF'?2.5:g==='MID'?1.8:g==='AM'?1.0:.55});
      const victim=weightedPick(victimPlayers,p=>{const g=playerPositionGroup(p);return g==='ST'?2.5:g==='AM'?2.0:g==='MID'?1.4:.55});
      if(!fouler||!victim)continue;
      const fm=minute();
      const attackingTeamId=foulingHome?m.awayId:m.homeId;
      const foulEv=addSimEvent(m,'foul',fm,fouler,null,0,{victimId:victim.id,foulingTeamId:foulingHome?m.homeId:m.awayId,attackingTeamId,source:'duel'});
      if(!foulEv)continue;
      const cardRoll=Math.random();
      let cardType=null;
      if(cardRoll<.006)cardType='red';
      else if(booked.has(Number(fouler.id))&&cardRoll<.075)cardType='secondYellow';
      else if(cardRoll<.285)cardType='yellow';
      if(cardType){
        if(cardType==='yellow')booked.add(Number(fouler.id));
        addSimEvent(m,cardType,fm,fouler,null,0,{source:'foul',relatedFoulId:foulEv.id,foulingTeamId:foulingHome?m.homeId:m.awayId});
      }
      // Jeder sichtbare Foulpfiff führt zu einem Freistoß; nur ein Teil davon ist direkt gefährlich.
      addSimEvent(m,'freeKick',fm,victim,null,0,{attackingTeamId,source:'foul',relatedFoulId:foulEv.id,dangerous:Math.random()<.28});
    }
    // Selten ein Elfmeter-Ereignis. Verwandelte Elfmeter werden als sichtbare Tore geplant.
    const hasPenalty=m.events.some(ev=>['penalty','missedPenalty'].includes(ev.type));
    if(!hasPenalty&&Math.random()<.10){
      const homePen=Math.random()<eventControlHome,sidePlayers=homePen?hXI:aXI,defPlayers=homePen?aXI:hXI,teamId=homePen?m.homeId:m.awayId;
      const shooter=pickScorerBySource(sidePlayers,'open'),victim=weightedPick(sidePlayers,p=>['ST','AM'].includes(playerPositionGroup(p))?2.4:1),fouler=weightedPick(defPlayers,p=>['DEF','MID'].includes(playerPositionGroup(p))?2.2:.8);
      if(shooter&&victim&&fouler){
        const pm=minute();
        const foulEv=addSimEvent(m,'foul',pm,fouler,null,0,{victimId:victim.id,foulingTeamId:homePen?m.awayId:m.homeId,attackingTeamId:teamId,source:'penalty-foul',inBox:true});
        if(Math.random()<.18)addSimEvent(m,'yellow',pm,fouler,null,0,{source:'foul',relatedFoulId:foulEv?.id||null});
        if(Math.random()<.76){if(homePen)hg=Math.min(8,hg+1);else ag=Math.min(8,ag+1);addSimEvent(m,'penalty',pm,shooter,null,0,{source:'penalty',attackingTeamId:teamId,relatedFoulId:foulEv?.id||null});}
        else addSimEvent(m,'missedPenalty',pm,shooter,null,0,{source:'penalty',attackingTeamId:teamId,relatedFoulId:foulEv?.id||null});
      }
    }
...
function openLiveSimulation(matchId){
  ...
  function runTo(node,x,y,ms=1200,cls='runner'){
    if(!node)return;const safeMs=Math.max(560,Number(ms)||1200);node.classList.add(cls);setLocked(node,safeMs+180);setNodeXY(node,x,y);later(()=>node.classList.remove(cls),safeMs+120);
  }
  ...
  function arrangeCorner(side,e,{causeAlreadyShown=false}={}){
    ...
    const takeCorner=()=>{
      ...
      later(()=>{showAction('KOPFBALL',playerLabel(aerial),'kommt zum Abschluss',side,1150*scale);const r=Math.random();const kind=r<.09?'goal':r<.50?'save':r<.63?'post':'chance';if(kind==='goal'){
  ...
  function realisticShot(side,shooter,{kind='chance',keeperNode=null,event=null,technique='auto',stationary=false}={}){
    const scale=tempoScale(),other=side==='home'?'away':'home',goalX=side==='home'?98.5:1.5,gk=keeperNode||keeper(other);actionBusyUntil=performance.now()+4100*scale;
    if(!shooter)return;
    ...
    later(()=>{
      ...
      flyBall(start,end,820*scale,{air:technique==='header'||distance>18,kind:kind==='chance'?'miss':kind==='post'?'post':'shot',onDone:()=>{
        ...
        if(kind==='goal'){showScene('⚽ Der Ball ist im Netz!',true);return}
        if(kind==='save'&&gk){
          registerVisibleEvent('save',side,event);
          const faux={type:'save',minute:Math.max(1,Math.floor(simSecond/60)),playerId:playerNodeId(gk),shotById:playerNodeId(shooter),attackingTeamId:side==='home'?m.homeId:m.awayId};
          ...
          const parry=Math.random()<.24;
          if(parry){
            showAction('PARADE',playerLabel(gk),'lenkt den Ball zur Ecke',other,1850*scale);showScene(`🧤 ${playerLabel(gk)} pariert zur Ecke`,true);
            later(()=>arrangeCorner(side,{playerId:playerNodeId(chooseRole(side,['AM','MID'])),attackingTeamId:side==='home'?m.homeId:m.awayId,source:'save'},{causeAlreadyShown:true}),850*scale);
          }else{
            possessionSide=other;possessionPassCount=0;possessionStartedAt=simSecond;setBallAtNode(gk);showAction('PARADE',playerLabel(gk),`hält den Schuss von ${playerLabel(shooter)}`,other,1850*scale);showScene(`🧤 ${playerLabel(gk)} hält sicher`,true);nextAmbientAction=simSecond+12+Math.random()*6;
          }
        }else if(kind==='post'){
          registerVisibleEvent('post',side,event);const faux={type:'post',minute:Math.max(1,Math.floor(simSecond/60)),playerId:playerNodeId(shooter),attackingTeamId:side==='home'?m.homeId:m.awayId};if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
          showAction('ALUMINIUM',playerLabel(shooter),'Pfosten oder Latte',side,1900*scale);showScene('🥅 Aluminium!',true);
          if(Math.random()<.38){possessionSide=side;const rebound=chooseRole(side,['ST','AM','MID'],[shooter])||shooter;if(rebound){setBallAtNode(rebound);nextAmbientAction=simSecond+4}}else{possessionSide=other;const outlet=gk||chooseRole(other,['DEF','MID']);if(outlet)setBallAtNode(outlet);nextAmbientAction=simSecond+10}
        }else{
          registerVisibleEvent('chance',side,event);const faux={type:'chance',minute:Math.max(1,Math.floor(simSecond/60)),playerId:playerNodeId(shooter),attackingTeamId:side==='home'?m.homeId:m.awayId};if(!event){tickerRows.unshift(liveSimTickerRow(faux,`🔥 CHANCE · ${playerLabel(shooter)}`,side));if(ticker)ticker.innerHTML=tickerRows.slice(0,6).join('')}
          showAction('VORBEI',playerLabel(shooter),'neben das Tor',side,1750*scale);showScene(`↗ ${playerLabel(shooter)} schießt vorbei`,true);possessionSide=other;possessionPassCount=0;possessionStartedAt=simSecond;const outlet=gk||chooseRole(other,['DEF','MID']);if(outlet)setBallAtNode(outlet);nextAmbientAction=simSecond+10+Math.random()*5;
        }
      }})
  ...
  function ambientAttack(){
    ...
    if(progress>70&&['ST','AM','MID'].includes(g)){
      ...
      if(['AM','MID'].includes(g)&&progress<82&&Math.random()<.32){realisticShot(side,carrier,{kind:Math.random()<.46?'save':Math.random()<.18?'post':'chance',keeperNode:keeper(side==='home'?'away':'home'),technique:'distance'});return}
      if(g==='ST'&&Math.random()<.24){dribble(carrier,side,true);nextAmbientAction=simSecond+8+Math.random()*5;return}
      realisticShot(side,carrier,{kind:Math.random()<.48?'save':Math.random()<.16?'post':'chance',keeperNode:keeper(side==='home'?'away':'home')});return
    }
    ...
    if(mustProgress){
      ...
      if(['MID','AM'].includes(g)&&progress>52&&Math.random()<.35){realisticShot(side,carrier,{kind:Math.random()<.48?'save':'chance',keeperNode:keeper(side==='home'?'away':'home'),technique:'distance'});return}
  ...
  function guaranteedGoalSequence(side,e){
    ...
    if(source==='corner'){
      ...
      showAction('ABGEFÄLSCHT',teamLabel(side),'vorheriger Abschluss wird zur Ecke geklärt',side,1100*scale);showScene('Abgefälscht – Ecke!',true);
      registerVisibleEvent('chance',side);registerVisibleEvent('corner',side,e);showSetpiece('ECKBALL');showAction('ECKE',teamLabel(side),`${playerLabel(taker)} auf ${playerLabel(shooter)}`,side,1700*scale);
  ...
  function focusEvent(e){
    ...
    if(e.type==='foul'){
      const fouler=nodeFor(e.playerId),victim=nodeFor(e.victimId),foulingSide=Number(e.foulingTeamId||0)===Number(m.homeId)?'home':'away';
      if(fouler&&victim){const vp=currentXY(victim);runTo(fouler,vp.x+(Math.random()-.5)*2.5,vp.y+(Math.random()-.5)*2.5,760*scale,'duel');victim.classList.add('duel');setLocked(victim,1700*scale)}
      registerVisibleEvent('foul',foulingSide,e);showAction('FOUL',fouler?playerLabel(fouler):'Foul',victim?`an ${playerLabel(victim)}`:'Freistoß',foulingSide,1500*scale);showScene(`Pfiff – Foul${victim?` an ${playerLabel(victim)}`:''}`,true);later(()=>{victim?.classList.remove('duel');eventSceneActive=false},1750*scale);return
    }
    if(e.type==='freeKick'){if(e.source!=='foul'&&e.source!=='penalty-foul'){eventSceneActive=false;return}arrangeFreeKick(side,e);later(()=>eventSceneActive=false,5200*scale);return}
    ...
    if(['yellow','secondYellow','red','injury'].includes(e.type)){if(['yellow','secondYellow','red'].includes(e.type)&&e.source!=='foul'){eventSceneActive=false;return}cinematicUntil=performance.now()+2700*scale;pitch?.classList.add('cinematic');later(()=>pitch?.classList.remove('cinematic'),2200*scale);const actor=nodeFor(e.playerId);if(actor){actor.classList.add('active');setLocked(actor,2300*scale);if(['red','secondYellow'].includes(e.type))later(()=>actor.classList.add('sent-off'),1800*scale)}showAction(e.type==='yellow'?'🟨 GELBE KARTE':e.type==='red'||e.type==='secondYellow'?'🟥 PLATZVERWEIS':'UNTERBRECHUNG',actor?playerLabel(actor):title,e.type==='secondYellow'?'Gelb-Rot – Spieler muss vom Feld':'',side,2600*scale);showScene(title,true);later(()=>eventSceneActive=false,2850*scale);return}
  ...
  function tick(now){
    ...
    try{
      if(!paused&&!ended){
        ...
        clock.textContent=`${String(Math.min(90,whole)).padStart(2,'0')}:${String(whole>=90?0:sec).padStart(2,'0')}`;
        ...
        // Bei 90:00 bleibt nur die Anzeige stehen; eventuell verspätete geplante Szenen werden noch sichtbar abgearbeitet.
        if(simSecond>=totalSeconds){
          simSecond=totalSeconds;clock.textContent='90:00';progress.style.width='100%';if(period)period.textContent='ENDE';
