...
import {getLeague,getSeason as getSeasonForState,getTeam,standingsAt,movementAt,maxMatchday} from "./standings632.js";
...
window.addEventListener("flm:recovered",()=>{
  setTimeout(()=>{try{toast("Spielstand automatisch repariert / wiederhergestellt")}catch{}},500);
});
...
function season(){ return getSeasonForState(state(),league()); }

// V100.3 compatibility bridge for the V89–V100 feature modules.
function getSeason(){
  const current=getSeasonForState(state(),league());
  if(current && !Array.isArray(current.fixtures) && Array.isArray(current.matches)) current.fixtures=current.matches;
  return current;
}
function getTeams(){ return Array.isArray(state()?.teams) ? state().teams : []; }
function getMyTeamId(){
  const s=state(),teams=getTeams();
  return Number(s?.manager?.managedTeamId ?? s?.settings?.managedTeamId ?? s?.myTeamId ?? teams[0]?.id ?? 0);
}
function buildTable(){
  const s=state();
  if(!s?.leagues?.length)return [];
  return standingsAt(s,maxMatchday(s)||null);
}
...
// ===== V101: CLUB BROWSER / EDITOR =====
const V101_KEY='fantasyLigaV101Custom';
let v101ClubId=null;
function v101Load(){try{return JSON.parse(localStorage.getItem(V101_KEY)||'null')||{clubs:{},players:{},world:{}}}catch{return{clubs:{},players:{},world:{}}}}
function v101Save(x){localStorage.setItem(V101_KEY,JSON.stringify(x))}
function v101Accrington(){return getTeams().find(t=>/accrington/i.test(t.name))||getTeams()[0]}
function v101CurrentClub(){if(v101ClubId==null)v101ClubId=v101Accrington()?.id;return v90Team(v101ClubId)||v101Accrington()}
function v101Club(t){
 const q=v101Load().clubs[String(t.id)]||{},raw=q.stadium??t.stadium;
 const base=(raw&&typeof raw==='object')?raw:{name:typeof raw==='string'?raw:'Vereinsstadion',capacity:t.stadiumCapacity||12000,image:''};
 return{name:q.name||t.name,stadium:(typeof q.stadium==='string'?q.stadium:(q.stadium?.name||base.name||'Vereinsstadion')),capacity:Number(q.capacity||q.stadium?.capacity||base.capacity||12000),image:q.stadiumImage||q.stadium?.image||base.image||'',strength:Number(q.strength||Math.round(v100TeamAverage(t)||teamAverage(t)||66))}
}
function v1005FixtureOrder(a,b){return Number(a.matchday??a.round??0)-Number(b.matchday??b.round??0)||String(a.date||'').localeCompare(String(b.date||''))||String(a.id||'').localeCompare(String(b.id||''))}
function v1005Played(f){return !!(f&&(f.played||f.done||f.status==='played'))}
function v1005ClubTimeline(teamId){
 const fs=(getSeason()?.fixtures||[]).filter(f=>String(f.homeId)===String(teamId)||String(f.awayId)===String(teamId)).slice().sort(v1005FixtureOrder);
 const played=fs.filter(v1005Played),future=fs.filter(f=>!v1005Played(f));
 return{all:fs,played,last:played.at(-1)||null,next:future[0]||null,recent:played.slice(-5)}
}
let v1005WorldDetail=false;
function v101Player(t,p,i){const id=v98PlayerId(t,p,i),q=v101Load().players[id]||{};return{id,name:q.name||p.name,rating:Number(q.rating??p.__v100Rating??p.rating??p.overall??60),position:q.position||p.position||p.pos||'Spieler'}}
function v101Picker(){const cur=v101CurrentClub();return `<div class="v101-picker"><button data-v101-prev>‹</button><select data-v101-club>${getTeams().map(t=>`<option value="${t.id}" ${String(t.id)===String(cur?.id)?'selected':''}>${v101Club(t).name}</option>`).join('')}</select><button data-v101-next>›</button></div>`}
function v101BindPicker(){const teams=getTeams(),move=d=>{let i=teams.findIndex(t=>String(t.id)===String(v101CurrentClub()?.id));v101ClubId=teams[(i+d+teams.length)%teams.length].id;profileTeamId=v101ClubId;render()};content.querySelector('[data-v101-prev]')?.addEventListener('click',()=>move(-1));content.querySelector('[data-v101-next]')?.addEventListener('click',()=>move(1));content.querySelector('[data-v101-club]')?.addEventListener('change',e=>{v101ClubId=e.target.value;profileTeamId=e.target.value;render()})}
function v101EditClub(t){const x=v101Load(),c=v101Club(t),name=prompt('Vereinsname:',c.name);if(name===null)return;const stadium=prompt('Stadionname:',c.stadium);if(stadium===null)return;const capacity=prompt('Fassungsvermögen:',c.capacity);if(capacity===null)return;const strength=prompt('Teamstärke:',c.strength);if(strength===null)return;x.clubs[String(t.id)]={...(x.clubs[String(t.id)]||{}),name:name.trim()||c.name,stadium:stadium.trim()||c.stadium,capacity:Math.max(100,Number(capacity)||c.capacity),stadiumImage:c.image||'',strength:Math.max(40,Math.min(99,Number(strength)||c.strength))};v101Save(x);render()}
function v101EditPlayer(t,p,i){const x=v101Load(),q=v101Player(t,p,i),name=prompt('Spielername:',q.name);if(name===null)return;const rating=prompt('Stärke 40–99:',q.rating);if(rating===null)return;const position=prompt('Position:',q.position);if(position===null)return;x.players[q.id]={name:name.trim()||q.name,rating:Math.max(40,Math.min(99,Number(rating)||q.rating)),position:position.trim()||q.position};v101Save(x);render()}
function v101WorldCfg(l){const x=v101Load(),q=x.world[l.id]||{},base=(l.teams||[]).map((t,i)=>({id:t.id||`${l.id}-${i}`,name:t.name||`Team ${i+1}`,strength:Number(t.strength||65)}));return{name:q.name||l.name,size:Number(q.size||base.length),teams:q.teams||base}}
function v101EditWorld(l){
 const x=v101Load(),c=v101WorldCfg(l),name=prompt('LigName:',c.name);if(name===null)return;
 let size=Math.max(4,Math.min(24,Math.round(Number(prompt('Anzahl Teams:',c.size))||c.size))),teams=c.teams.slice(0,size);
 while(teams.length<size)teams.push({id:`${l.id}-c${teams.length}`,name:`Team ${teams.length+1}`,strength:65});
 x.world[l.id]={name:name.trim()||c.name,size,teams};v101Save(x);v1005ApplyWorldConfig(l,x.world[l.id],true);render()
}
function v1005ApplyWorldConfig(l,cfg,reset=false){
 if(!l||!cfg)return;
 const teams=(cfg.teams||[]).slice(0,cfg.size||cfg.teams.length).map((t,i)=>({id:t.id||`${l.id}-c${i}`,name:t.name||`Team ${i+1}`,strength:Math.max(40,Math.min(99,Number(t.strength)||65)),country:l.country}));
 const signature=teams.map(t=>`${t.id}:${t.name}:${t.strength}`).join('|'),oldSig=(l.teams||[]).map(t=>`${t.id}:${t.name}:${t.strength}`).join('|');
 l.name=cfg.name||l.name;
 if(reset||signature!==oldSig){l.teams=teams;l.rounds=v91Schedule(teams.map(t=>t.id));l.results=[]}
}
function v101EditWorldTeam(l,i){const x=v101Load(),c=v101WorldCfg(l),t=c.teams[i];if(!t)return;const name=prompt('Mannschaftsname:',t.name);if(name===null)return;const st=prompt('Teamstärke 40–99:',t.strength);if(st===null)return;t.name=name.trim()||t.name;t.strength=Math.max(40,Math.min(99,Number(st)||t.strength));x.world[l.id]=c;v101Save(x);v1005ApplyWorldConfig(l,c,true);render()}
  ...
  el("#app").innerHTML=`
    <div class="shell v96-shell">
      <header class="topbar"><div class="topbar-inner">
        <div class="brand"><div class="brandmark">🏆</div><div>Fantasy Liga Elite <span class="version-pill">V100.5 · Club/Welt</span></div></div>
...
function v91MainMatchday(){
 const s=getSeason(),fs=s?.fixtures||[],played=fs.filter(f=>f.played||f.done||f.status==='played');
...
function v91SyncWorld(){
 const w=v91LoadWorld(),target=Math.max(0,v91MainMatchday()),custom=v101Load().world||{};
 Object.values(w.leagues).forEach(l=>{
   if(custom[l.id])v1005ApplyWorldConfig(l,custom[l.id],false);
...
function v94TeamMatches(id,mode='overall'){
 ...
 }).sort((a,b)=>Number(a.matchday??a.round??0)-Number(b.matchday??b.round??0)
...
function v93MainSnapshot(){
 const season=getSeason(),fs=season?.fixtures||[],played=fs.filter(f=>f.played||f.done||f.status==='played'),allDone=fs.length>0&&played.length===fs.length;
...
function renderFootballWorld(){
 const w=v91SyncWorld(),leagues=Object.values(w.leagues).slice(0,4),l=w.leagues[worldLeagueId]||leagues[0];
 if(!v1005WorldDetail){
   content.innerHTML=`<section class="v91-world v1005-world-home">
    <header class="v91-hero"><div><div class="v91-kicker">FUSSBALLWELT</div><h1>Deine vier Ligen.</h1><p>Jede Liga läuft parallel zu deiner Hauptliga. Namen, Teams, Stärke und Ligagröße bestimmst du selbst.</p></div><div class="v91-day"><small>AKTUELLER SPIELTAG</small><b>${w.matchday}</b></div></header>
    <div class="v1005-world-buttons">${leagues.map(x=>{const c=v101WorldCfg(x),tb=v91Table(x);return `<button data-open-world="${x.id}"><span>${x.country}</span><h2>${c.name}</h2><small>${c.size} Teams · ST ${Math.min(w.matchday,x.rounds.length)} / ${x.rounds.length}</small><b>${tb[0]?.name||'Noch keine Tabelle'} →</b></button>`}).join('')}</div>
    <div class="v1005-world-note">Tippe eine Liga an. Dort findest du Tabelle, Spielplan, Ergebnisse, Torjäger und die Bearbeitung.</div>
   </section>`;
   content.querySelectorAll('[data-open-world]').forEach(b=>b.onclick=()=>{worldLeagueId=b.dataset.openWorld;v1005WorldDetail=true;render()});
   return
 }
 const table=v91Table(l),scorers=v91TopScorers(l),cfg=v101WorldCfg(l),current=Math.min(w.matchday,l.rounds.length),nextMd=Math.min(current+1,l.rounds.length),next=l.rounds[nextMd-1]||[];
 content.innerHTML=`<section class="v91-world v1005-world-detail">
  <button class="v90-back" data-world-back>← Zurück zu den 4 Ligen</button>
  <div class="v91-head"><div><span>${l.country}</span><h2>${cfg.name}</h2><small>${cfg.size} Teams · Saison ${l.season}</small></div><button class="v1005-edit-league" data-v101-world="${l.id}">Liga bearbeiten</button></div>
  <nav class="v1005-world-switch">${leagues.map(x=>`<button data-world-league="${x.id}" class="${x.id===l.id?'active':''}">${v101WorldCfg(x).name}</button>`).join('')}</nav>
  <div class="v91-grid">
   <article class="v91-card v91-table-card"><h3>Tabelle · ST ${current}</h3><div class="v91-table">${table.map((r,i)=>`<div class="${i<3?'top':''}"><em>${i+1}</em><b>${r.name}</b><span>${r.p}</span><span>${r.gf-r.ga>0?'+':''}${r.gf-r.ga}</span><strong>${r.pts}</strong></div>`).join('')}</div></article>
   <div class="v91-side"><article class="v91-card"><h3>${current<l.rounds.length?`Nächster Spieltag ${nextMd}`:'Saison beendet'}</h3><div class="v91-fixtures">${current<l.rounds.length?next.map(g=>`<div><span>${l.teams.find(t=>t.id===g.homeId)?.name}</span><b>–</b><span>${l.teams.find(t=>t.id===g.awayId)?.name}</span></div>`).join(''):'<p>Alle Spieltage gespielt.</p>'}</div></article>
   <article class="v91-card"><h3>Torjäger</h3><div class="v91-scorers">${scorers.length?scorers.slice(0,5).map((p,i)=>`<div><em>${i+1}</em><span><b>${p.name}</b><small>${p.team}</small></span><strong>${p.goals}</strong></div>`).join(''):'<p>Noch keine Tore.</p>'}</div></article></div>
  </div>
  <article class="v91-card"><h3>Teams bearbeiten</h3><p class="v90-help">Name und Stärke antippen. Die Anzahl der Teams stellst du oben über „Liga bearbeiten“ ein.</p><div class="v1005-world-team-editor">${cfg.teams.map((t,i)=>`<button data-v101-world-team="${i}"><span>${t.name}</span><b>Stärke ${t.strength}</b></button>`).join('')}</div></article>
  <article class="v91-card"><h3>Letzte Ergebnisse</h3><div class="v91-results">${l.results.slice().sort((a,b)=>b.matchday-a.matchday).slice(0,10).map(f=>`<div><span>${l.teams.find(t=>t.id===f.homeId)?.name}</span><strong>${f.homeGoals}:${f.awayGoals}</strong><span>${l.teams.find(t=>t.id===f.awayId)?.name}</span><small>ST ${f.matchday}</small></div>`).join('')||'<p>Noch keine Ergebnisse.</p>'}</div></article>
  <div class="v1005-world-actions"><button class="v92-open" data-v92-open>🏆 Pokal & Auf-/Abstieg</button><button class="v93-open" data-v93-open>📚 Saison-Historie</button></div>
 </section>`;
 content.querySelector('[data-world-back]')?.addEventListener('click',()=>{v1005WorldDetail=false;render()});
 content.querySelectorAll('[data-world-league]').forEach(b=>b.onclick=()=>{worldLeagueId=b.dataset.worldLeague;render()});
 content.querySelector('[data-v101-world]')?.addEventListener('click',()=>v101EditWorld(l));
 content.querySelectorAll('[data-v101-world-team]').forEach(b=>b.onclick=()=>v101EditWorldTeam(l,Number(b.dataset.v101WorldTeam)));
...
function v90ClubMatches(id){return v90Played().filter(f=>String(f.homeId)===String(id)||String(f.awayId)===String(id)).sort(v1005FixtureOrder)}
...
function v90PlayerStats(p){
 const season=getSeason(),all=season?.playerStats||season?.stats?.players||{},id=p?.id??p?.playerId??p?.name;
 const seasonStats=Array.isArray(all)?all.find(x=>String(x.playerId??x.id??x.name)===String(id)):(all[id]||all[p?.name]||null);
 const st=seasonStats||p?.stats||{};
 return{apps:Number(st.apps??st.appearances??st.matches??0),goals:Number(st.goals??0),assists:Number(st.assists??0),yellow:Number(st.yellow??st.yellowCards??0),red:Number(st.red??st.redCards??0)}
}
function openClubProfile(id){profileTeamId=id;profilePlayerId=null;view='clubProfile';render()}
function openV90PlayerProfile(teamId,playerId){profileTeamId=teamId;profilePlayerId=playerId;view='playerProfile';render()}
function renderClubProfile(){
 const team=v90Team(profileTeamId)||v101CurrentClub();if(!team){view='home';return renderView()}
 ...
 const timeline=v1005ClubTimeline(team.id),fixtures=timeline.all,next=timeline.next,opp=next?v90Team(String(next.homeId)===String(team.id)?next.awayId:next.homeId):null;
 content.innerHTML=`<section class="v90-world">${v101Picker()}<button class="v90-back" data-v90="back">← Zurück</button>
 ...
 <section class="v1005-stadium-profile ${v101Club(team).image?'has-image':''}" style="${v101Club(team).image?`background-image:linear-gradient(90deg,rgba(3,15,26,.88),rgba(3,15,26,.22)),url('${v101Club(team).image}')`:''}"><div><small>STADION</small><h2>${v101Club(team).stadium}</h2><b>${v101Club(team).capacity.toLocaleString('de-DE')} Plätze</b></div><button data-v101-edit-club>Verein bearbeiten</button></section><div class="v1005-club-actions"><button class="v98-open" data-v98-open>⇄ Transfers & Entwicklung</button></div><div class="v90-profile-grid"><article class="v90-card"><h3>Saisonbilanz</h3><div class="v90-numbers"><div><b>${rec.p}</b><span>Spiele</span></div><div><b>${rec.w}</b><span>Siege</span></div><div><b>${rec.d}</b><span>Remis</span></div><div><b>${rec.l}</b><span>Niederlagen</span></div><div><b>${rec.gf}:${rec.ga}</b><span>Tore</span></div></div></article>
 ...
 <details class="v90-card v1005-player-details"><summary>✎ Spieler bearbeiten <span>öffnen</span></summary><div class="v101-player-editor">${v100VisibleSquad(team).map((pl,i)=>{const q=v101Player(team,pl,i);return `<button data-v101-edit-player="${i}"><span><b>${q.name}</b><small>${q.position}</small></span><strong>${q.rating}</strong></button>`}).join('')}</div></details><article class="v90-card"><h3>Kader</h3><div class="v90-squad-list">${squad.length?squad.slice().sort((a,b)=>Number(b.rating??b.overall??0)-Number(a.rating??a.overall??0)).map(p=>`<button data-player="${String(p.id??p.playerId??p.name).replace(/"/g,'&quot;')}"><span><b>${p.name}</b><small>${p.position||p.pos||'Spieler'}</small></span><strong>${p.rating??p.overall??'–'}</strong></button>`).join(''):'<p class="muted">Keine Kaderspieler hinterlegt.</p>'}</div></article>
 ...
 content.querySelectorAll('[data-player]').forEach(b=>b.addEventListener('click',()=>openV90PlayerProfile(team.id,b.dataset.player)));
 content.querySelectorAll('[data-h2h]').forEach(b=>b.addEventListener('click',()=>{const other=v90Team(b.dataset.h2h),x=v90H2H(team.id,b.dataset.h2h),el=content.querySelector('#v90-h2h');if(el)el.innerHTML=`<div class="v90-h2h-box"><b>${team.name}</b><strong>${x.a} – ${x.d} – ${x.b}</strong><b>${other?.name||''}</b><small>Siege · Remis · Siege (${x.matches.length} Spiele)</small></div>`}));
  v101BindPicker();
 content.querySelector('[data-v101-edit-club]')?.addEventListener('click',()=>v101EditClub(team));
 content.querySelectorAll('[data-v101-edit-player]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.v101EditPlayer),pl=v100VisibleSquad(team)[i];if(pl)v101EditPlayer(team,pl,i)});
}
...
function renderHome(){
  const season=getSeason(),teams=getTeams(),club=v101CurrentClub(),myId=club?.id;
  ...
  const fixtures=season.fixtures||[],timeline=v1005ClubTimeline(club.id),clubPlayed=timeline.played;
  const last=timeline.last,next=timeline.next;
  const recent=timeline.recent.map(f=>{
  ...
  content.innerHTML=`
    <section class="manager-hub">${v101Picker()}<div class="mgr-hero">
      ...
      <section class="v1005-stadium-hero ${v101Club(club).image?'has-image':''}" style="${v101Club(club).image?`background-image:linear-gradient(90deg,rgba(4,17,29,.88),rgba(4,17,29,.28)),url('${v101Club(club).image}')`:''}"><div><small>STADION</small><b>${v101Club(club).stadium}</b><span>${v101Club(club).capacity.toLocaleString('de-DE')} Plätze</span></div></section><div class="mgr-grid">
...
function openLeagueEditor(id){
  ...
  const active=id?getSeasonForState(state(),l):{name:"2026/27"};
